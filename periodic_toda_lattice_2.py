...
class Particle:
    ...
    def I_vec(self):
        V = + np.exp(self.q1-self.q2) + np.exp(self.q2-self.q1)
        H = 0.5*self.p1**2 + 0.5*self.p2**2 + V
        H1 = -0.5*(self.p1 + self.p2)
        J1 = (self.p1 - self.p2)**2 + 4*V
...
def Toda_V_kick(p, dt):
    p.p1 += dt * (- np.exp( ( p.q1 - p.q2 ) ) + np.exp( ( p.q2 - p.q1 ) ) )
    p.p2 += dt * (+ np.exp( ( p.q1 - p.q2 ) ) - np.exp( ( p.q2 - p.q1 ) ) )
    #p.p1 += dt * ( np.exp( - ( p.q1 - p.q0 ) ) - np.exp( - ( p.q2 - p.q1 ) ) )
    #p.p2 += dt * ( np.exp( - ( p.q2 - p.q1 ) ) - np.exp( - ( p.q3 - p.q2 ) ) )
    return

def Toda_Lin_kick(p, dt, LTK):
    p.p1 += dt * ( LTK.kick_p1(p.q1, p.q2) - LTK.kick_p1(p.q2, p.q1) )
    p.p2 += dt * ( LTK.kick_p2(p.q1, p.q2) - LTK.kick_p2(p.q2, p.q1) )
    return

def Toda_Cub_kick(p, dt, CTK):
    p.p1 += dt * ( CTK.kick_p1(p.q1, p.q2) - CTK.kick_p1(p.q2, p.q1) )
    p.p2 += dt * ( CTK.kick_p2(p.q1, p.q2) - CTK.kick_p2(p.q2, p.q1) )
    return


NN = 21
LTK  = LinTodaKick(NN,NN, x0=15, y0=15)
#CTK  = LTK
CTK  = TricubTodaKick(NN,NN, x0=15, y0=15)

part = Particle(0.5, 0., -0.5, 0.)
part_lin = Particle(0.5, 0., -0.5, 0.)
part_cub = Particle(0.5, 0., -0.5, 0.)
...
ax41.plot(TT, x_tbt[:,0] - x_tbt[:,2])
ax41.plot(TT, x_tbt_lin[:,0] - x_tbt_lin[:,2])
ax41.plot(TT, x_tbt_cub[:,0] - x_tbt_cub[:,2])

print(f"Exact rms(IoM): {np.std(I_tbt[:,:-1], axis=0)}")
print(f"Linear rms(IoM): {np.std(I_tbt_lin[:,:-1], axis=0)}")
print(f"Cubic rms(IoM): {np.std(I_tbt_cub[:,:-1], axis=0)}")
plt.show()
